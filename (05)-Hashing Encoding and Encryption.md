...
يعتمد على مفتاح واحد لتشفير وفك تشفير البيانات. من أبرز الخوارزميات:

### AES (Advanced Encryption Standard)
تُستخدم لتأمين البيانات في التطبيقات المالية. تدعم مفاتيح 128، 192، و256 بت. على سبيل المثال، إذا كان لديك ملف يحمل معلومات حساسة مثل "MySecretPassword"، قد يصبح بعد التشفير "3D3B5C8F69F489C9DA5F19C0B1234ABC"، مما يجعله غير قابل للقراءة بدون المفتاح.

### DES (Data Encryption Standard)
يتطلب مفتاحًا بطول 56 بت. رغم أنه كان شائعًا، إلا أنه ضعيف الآن. على سبيل المثال، قد يُستخدم لتشفير "HelloWorld" إلى قيمة غير قابلة للقراءة.

### 3DES (Triple DES)
يُطبق DES ثلاث مرات لزيادة الأمان. على سبيل المثال، "MyPassword" قد يتحول إلى قيمة مثل "A6B8C9D0E1F2G3H4".

### Twofish
تدعم مفاتيح تصل إلى 256 بت وتُستخدم في التطبيقات عالية الأداء. على سبيل المثال، قد يتحول النص "SensitiveData" إلى "9C8B7A6F5E4D3C2B".

### Blowfish
تدعم مفاتيح من 32 إلى 448 بت وتُستخدم في تشفير الملفات. على سبيل المثال، "SecretInfo" قد يصبح "1F2E3D4C5B6A7F8E".

## 2. Asymmetric Encryption

**Asymmetric Encryption**

يستخدم زوجًا من المفاتيح: عام للتشفير وخاص لفك التشفير. من أبرز الخوارزميات:

### RSA (Rivest–Shamir–Adleman)
تعتمد على تحليل الأعداد الكبيرة. إذا أراد شخص إرسال "Hello"، يستخدم المفتاح العام لتشفيرها إلى "XyZ123456ABC"، فقط المستلم بمفتاحه الخاص يمكنه فك التشفير.

### DH (Diffie-Hellman)
يُستخدم لتبادل المفاتيح بشكل آمن. الطرفان يمكنهما إنشاء مفتاح مشترك دون تبادل المفتاح مباشرة.

### DSA (Digital Signature Algorithm)
تُستخدم لتوقيع البيانات رقمياً، مما يضمن أن المستند أصلي.

### ECDH (Elliptic Curve Diffie-Hellman)
تُعتبر نسخة محسنة من DH، تتيح تبادل مفاتيح أكثر أمانًا باستخدام مفاتيح أصغر.

إذا كان لديك أي استفسارات، فلا تتردد في إخباري!
